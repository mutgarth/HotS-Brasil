...
class BuildsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create build" do
    assert_difference('Build.count') do
      post builds_url, params: { build: { description: @build.description, title: @build.title } }
    end
  ...
  test "should update build" do
    patch build_url(@build), params: { build: { description: @build.description, title: @build.title } }
